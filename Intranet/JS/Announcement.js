...
var AnnouncementHTMLId = 'AnnouncementsDiv';
...
function getAnnouncement(){
    date = new Date();
    var Announcementsurl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('"+AnnouncementsListName+"')/items?$top=5&$select=*,AttachmentFiles,Title&$expand=AttachmentFiles&$filter=Expires ge '"+convertDateSharePointFormat(date)+"'&$orderBy=Id desc";
    getItems(Announcementsurl,function(data){
        ...
        if(AnnouncementResults.length > 0){
            for(j=0;j<AnnouncementResults.length;j++){
                ...
                var HTMLToAppendForAnnouncement = '<li title="'+(AnnouncementResults[j].Body == null ? AnnouncementResults[j].Title : AnnouncementResults[j].Body)+'">'+
                                    '<img src="'+Image+'">'+
